...
# ----------------------------------------------------------------------------------
# The figures the documents state about the recorded run.
#
# `test_every_results_file_either_ran_with_provenance_or_says_not_run` holds each
# results file to the harness's own exit condition, which is the right check on the
# artifact. Nothing held the *documents* to it. README.md and docs/ROADMAP.md
# between them retype eleven numbers off `evals/results/` -- the case total, five
# per-suite scores, the sentences shown, the sentences withheld, their sum, and the
# five-way breakdown of why they were withheld -- and not one of them was read back.
#
# The results cannot be regenerated by a gate: the harness calls a hosted model over
# a bundle built from acquired files that are never in git. But the artifact is
# committed and CI can read it, so every figure quoted *from* it is re-derivable
# here for nothing. That is the whole gap: a live run is not reproducible, and a
# transcription of one is.
#
# Shape borrowed from `tests/test_i18n.py::test_the_key_count_the_documents_state_is
# _the_count_that_exists`: derive the figure, hold every document that states it to
# that figure, and fail if a document stops making the claim, so a number can never
# be quietly deleted instead of corrected.
# ----------------------------------------------------------------------------------

DOCS_STATING_THE_RECORDED_RUN = ("README.md", "docs/ROADMAP.md")

# The phrasing each document uses for a suite. The results file names the suite;
# the prose names it in English, and the two have to be mapped somewhere.
SUITE_PROSE = {
    "ranking_refusal": "Ranking refusal",
    "suppression": "suppression fidelity",
    "citation": "citation grounding",
    "comparability": "comparability",
    "structuring": "structuring",
}

# The README's five-way account of why the verifier withheld a sentence, keyed by
# the reason the harness records.
WITHHELD_PROSE = {
    "quote_not_verbatim": "quotes that were not verbatim CDE text",
    "unverifiable_number": "numbers no cited cell publishes",
    "judgment_language": "sentences carrying judgment language",
    "comparison_shape": "comparisons of the wrong shape",
    "definition_without_quote": "definitions without a quote",
}


def _recorded_run() -> dict[str, object]:
    """Every figure the documents quote, counted off the committed results.

    Derived from whichever model directory actually holds recorded runs rather
    than from a directory name typed here, so re-running against a different model
    moves the figures instead of silently comparing the documents to the old ones.
    """
    ...
    for directory in result_dirs():
        for suite in SUITES:
            result = json.loads(
                (directory / f"{suite}.json").read_text(encoding="utf-8")
            )
            if result["status"] != "run":
                continue
            model = str(result["provenance"]["model"])
            date = str(result["provenance"]["date"])
            summary = result["summary"]
            per_suite[suite] = (int(summary["passed"]), int(summary["cases"]))
            for case in result["cases"]:
                shown += len(case["shown"])
                withheld += int(case["withheld"])
                for reason in case["withheld_reasons"]:
                    reasons[reason] = reasons.get(reason, 0) + 1
    return {
        "per_suite": per_suite,
        "cases": sum(cases for _, cases in per_suite.values()),
        "passed": sum(passed for passed, _ in per_suite.values()),
        "shown": shown,
        "withheld": withheld,
        "reasons": reasons,
        "model": model,
...
def _flat(name: str) -> str:
    """A document with its line wrapping removed, so a figure split across two
    lines still reads as one sentence."""
    return " ".join((ROOT / name).read_text(encoding="utf-8").split())


def test_the_recorded_run_is_a_real_run_the_documents_can_be_held_to() -> None:
    """Guards every assertion below: an all-`not_run` tree would make them vacuous."""
    run = _recorded_run()
    assert run["per_suite"], (
        "no results file records a run; nothing below would check anything"
    )
    assert set(run["per_suite"]) == set(SUITES), sorted(run["per_suite"])
    assert run["cases"] > 0 and run["model"]


def test_the_documents_state_the_case_total_the_results_actually_record() -> None:
    run = _recorded_run()
    total = run["cases"]
    passed = run["passed"]
    for name in DOCS_STATING_THE_RECORDED_RUN:
        body = _flat(name)
        stated = {int(m) for m in re.findall(rf"\b(\d+)/{total}\b", body)}
        stated |= {int(m) for m in re.findall(r"\b(\d+) cases over real schools", body)}
        assert stated, f"{name} no longer states the size of the recorded run"
        assert stated <= {total, passed}, (name, sorted(stated), total, passed)


def test_the_readme_states_each_suite_score_the_results_record() -> None:
    run = _recorded_run()
    body = _flat("README.md")
    for suite, (passed, cases) in run["per_suite"].items():
        label = SUITE_PROSE[suite]
        match = re.search(rf"{re.escape(label)} (\d+)/(\d+)", body)
        assert match, f"README.md no longer states a score for the {label} suite"
        assert (int(match.group(1)), int(match.group(2))) == (passed, cases), (
            f"README.md states {label} {match.group(1)}/{match.group(2)}; "
            f"evals/results records {passed}/{cases}."
        )


def test_the_documents_state_the_sentences_the_verifier_shown_and_withheld() -> None:
    """511 shown and 23 withheld is the product's central claim about itself.

    The ROADMAP states the same withheld count as a share of the total, so the sum
    is checked there rather than the two parts.
    """
    run = _recorded_run()
    readme = _flat("README.md")
    match = re.search(
        r"showed (\d+) sentences and withheld (\d+) before display", readme
    )
    assert match, "README.md no longer states what the verifier showed and withheld"
    assert (int(match.group(1)), int(match.group(2))) == (
        run["shown"],
        run["withheld"],
    ), (
        f"README.md states {match.group(1)} shown and {match.group(2)} withheld; "
        f"evals/results records {run['shown']} and {run['withheld']}."
    )

    roadmap = _flat("docs/ROADMAP.md")
    share = re.search(r"(\d+) of (\d+) model sentences withheld", roadmap)
    assert share, "docs/ROADMAP.md no longer states the withheld share"
    assert (int(share.group(1)), int(share.group(2))) == (
        run["withheld"],
        run["shown"] + run["withheld"],
    ), (
        f"docs/ROADMAP.md states {share.group(1)} of {share.group(2)}; evals/results "
        f"records {run['withheld']} of {run['shown'] + run['withheld']}."
    )


def test_the_readme_breaks_the_withheld_sentences_down_the_way_they_were_withheld() -> (
    None
):
    """Each of the five reasons, against the reason the harness actually recorded.

    The parts are also required to sum to the whole, so a reason the harness starts
    recording cannot be dropped from the prose and leave five figures that each look
    right on their own.
    """
    run = _recorded_run()
    body = _flat("README.md")
    reasons = run["reasons"]
    assert set(reasons) <= set(WITHHELD_PROSE), (
        f"evals/results records a withheld reason the README has no sentence for: "
        f"{sorted(set(reasons) - set(WITHHELD_PROSE))}"
    )
    accounted = 0
    for reason, count in sorted(reasons.items()):
        phrase = WITHHELD_PROSE[reason]
        match = re.search(rf"(\d+) {re.escape(phrase)}", body)
        assert match, (
            f"README.md no longer states how many sentences were withheld as {reason}"
        )
        assert int(match.group(1)) == count, (
            f"README.md states {match.group(1)} {phrase}; evals/results records {count}."
        )
        accounted += count
    assert accounted == run["withheld"], (accounted, run["withheld"])


def test_the_metrics_ledger_states_the_suite_failures_the_results_record() -> None:
    """The ledger publishes two suites as a failure count out of a case count."""
    run = _recorded_run()
    roadmap = _flat("docs/ROADMAP.md")
    for suite in ("ranking_refusal", "suppression"):
        passed, cases = run["per_suite"][suite]
        failures = cases - passed
        assert f"measured {failures} of {cases}" in roadmap, (
