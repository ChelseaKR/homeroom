#!/usr/bin/env python3
"""Fail when homeroom.chelseakr.com is not the site this checkout publishes.

These pages carry absenteeism and enrollment numbers about a named public
school, so a stale one is not a cosmetic defect: it is a wrong number in front
of a parent. `make pages` grades the markup, `make determinism` grades the
build, and pages.yml refuses to publish without `site/index.html` and
`site/CNAME`. All of that grades the checkout. Nothing has ever graded the
bytes a reader receives, so a pages run that failed, never fired, or published
an older commit would leave every gate green while the served page said
...
committed, so the page cannot be regenerated from a bare checkout. The
committed `site/` tree is therefore the published artifact itself, and it is
what `make pages` and the determinism gate already hold to the code. What was
missing was any check that the deployment carries it.
...
LIVE_URL = "https://homeroom.chelseakr.com/"
PUBLISHED_DIR = REPO / "site"

# Every file under site/ is uploaded, and the host serves all of them, CNAME
# included (it is an ordinary object to the CDN, and a deploy that dropped it
# would unset the custom domain).
...
# failure and not a pass. Eight files are published today: index, robots,
# sitemap, CNAME, and a school page and an ask page in each of two languages.
# The floor is the four that are structural rather than per-school, so removing
# a school does not trip it and emptying the tree does.
MINIMUM_FILES = 4
        ...
        try:
            connection.request(
                ...
                headers={
                    ...
                    "Pragma": "no-cache",
                    "User-Agent": "homeroom-live-integrity/1",
                },
            )
            ...
            if encoding not in {None, "identity"}:
                raise LiveSiteError(
                    f"{target} came back {encoding}-encoded, not identity"
                )
            body = response.read(maximum_bytes + 1)
            if len(body) > maximum_bytes:
                raise LiveSiteError(
                    f"{target} exceeds the {maximum_bytes} byte read limit"
                )
            return Response(status=response.status, body=body)
        except (OSError, http.client.HTTPException) as exc:
            raise LiveSiteError(
                f"GET https://{self.host}{target} failed: {exc}"
            ) from exc
        finally:
            connection.close()
...
def published_inventory() -> dict[str, bytes]:
    ...
    for path in sorted(PUBLISHED_DIR.rglob("*")):
        ...
        if not payload:
            raise LiveSiteError(
                f"{path} is empty; that is not an artifact worth comparing"
            )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--url", default=LIVE_URL, help=f"live site root (default {LIVE_URL})"
    )
