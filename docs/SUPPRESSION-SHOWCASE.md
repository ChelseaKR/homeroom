...
does. The denominator is not the same for every category, and this table used to
imply it was: `TA` and the nine race and ethnicity codes have a row at 9,801 of
the 10,534 active schools, but `GX` (Non-binary) has one at 2,045 and `SF`
(Foster youth) at 7,312. So each row carries its own denominator, and the share
is of that row's own denominator, never of 10,534.

| Category | With any row | Published | Withheld | Withheld, of schools with any row |
|---|---|---|---|---|
| `GX` (Non-binary) | 2,045 | 55 | 1,990 | 97.3% |
| `RI` (American Indian or Alaska Native) | 9,801 | 451 | 9,350 | 95.4% |
| `RP` (Pacific Islander) | 9,801 | 528 | 9,273 | 94.6% |
| `SF` (Foster youth) | 7,312 | 946 | 6,366 | 87.1% |
| `RD` (race or ethnicity not reported) | 9,801 | 1,995 | 7,806 | 79.6% |
| `RF` (Filipino) | 9,801 | 2,908 | 6,893 | 70.3% |
| `RB` (African American) | 9,801 | 4,864 | 4,937 | 50.4% |
| `TA` (all students) | 9,801 | 9,718 | 83 | 0.8% |

`GX` is the most-withheld category in the file, and until 2026-08-29 no document
in this repository named it: the README, the ROADMAP ledger, the risk register
and this table all called `RI` the maximum, which it is not. `GX` is a rendered
subgroup, not a parse-only code -- `homeroom.profiles.ABSENTEEISM_SUBGROUP_FAMILIES`
puts it in the `gender` family, and it is on Birch Lane's published page in both
languages, reading *withheld to protect privacy* beside 55 / 1,990 / 8,489.

Its 97.3% is not the same kind of statement as `RI`'s 95.4%, and the difference
belongs in the sentence rather than in a footnote. `RI` is withheld at 9,350 of
the 9,801 schools that report it at all, which is nearly every active school.
`GX` is withheld at 1,990 of only 2,045; the other 8,489 active schools have no
Non-binary row for CDE to withhold, so the 97.3% describes a much smaller slice
of the state. Both are true, and neither is the whole picture on its own.

Three categories -- `GX`, `RI` and `RP` -- are withheld for over 94% of the
schools that report them at all.
