...
| User-visible strings carried in both languages | 190 keys per locale, 380 strings total (114 interface, 33 reporting categories, 14 grade spans, 4 subgroup families, 25 chronic-absenteeism categories); 122 keys and 71 interface at M4, before D3 added its own 25-code catalog and 10 interface strings, and 157 before the ask layer (ADR 0003) added 33 fixed interface strings: its labels, every refusal, and the ask page's own copy, none of which the model writes | `src/homeroom/i18n.py` |
