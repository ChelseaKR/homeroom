...
| M4 (done 2026-08-08) | First bilingual school page, with district and statewide context | One real school rendered EN/ES from acquired data (Birch Lane Elementary, Davis Joint Unified, CDS 57726786056246); a11y and EN/ES parity gates wired from this milestone and failing the run, though not required on `main` and so not merge-blocking (RR-12); each measure sits beside its district and statewide figure, read from CDE's own `Charter=ALL` aggregate rows and never summed from schools |
...
| The saving that was taken, on the four page kinds the refusal does not reach | -117.1 MB: the `<style>` block is 118,190,092 bytes across 23,305 files, 13.62% of the tree, in four blocks (5,061 x 21,068 school, 5,169 x 2,234 browse, 5,250 landing, 6,074 x 2 ask). The first three now link one 5,341-byte `homeroom.css` at 44 bytes of `<link>` a page. Counted, not yet published: `site/` is still the 867.6 MB tree until the owner reruns `make publish`. It takes the tree to 750.5 MB (75.0%), and a D5 republish on top to 934.3 MB -- inside the 1 GB cap and still over the 900 MB budget above | `site/`, 2026-09-07; issue #95 |
...
- Accessibility: applies as of M4, gates wired and failing the run, though not
  required on `main` and so not merge-blocking, RR-12 (html-validate
